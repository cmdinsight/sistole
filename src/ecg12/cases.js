...
export const CASES = [
...

  {
    ...
    record: '4647',
    age: 48, sex: 'M',
    vitals: { bp: '126/80', spo2: 98, rr: 14 },
    highlight: ['II', 'V2', 'V3'],
    answer: 'extrasistole',
    metrics: { kind: 'pvc' },
    // Medido: un latido prematuro de forma distinta. Llegó al 60 % del ciclo
    // —o sea, mucho antes de tiempo— y se parece un 92 % a la plantilla del
    // latido típico, contra el 99 % o 100 % de todos los demás. FC 68, cv 0,018.
    //
    // El QRS general da 112 ms, más ancho de lo que corresponde a este trazado:
    // el latido promedio incluye la extrasístole y la extrasístole es ancha. Es
    // un efecto real de la medición y el caso lo dice, en vez de esconderlo.
    findings: {
      prematureBeats: [1, 2],
      rate: [58, 78],
      irregular: false,
    ...
    stem: {
      es: 'Hombre de 48 años, sin antecedentes, que consulta por «vuelcos» en el pecho que nota sobre todo cuando se acuesta y se queda quieto. No tiene dolor, ni falta de aire, ni se ha desmayado. Corre tres veces por semana y dice que corriendo no los siente. El examen físico es normal.',
      en: '48-year-old man, no medical history, who comes because of "flip-flops" in his chest that he notices especially when lying down and keeping still. No pain, no breathlessness, never fainted. He runs three times a week and says he does not feel them while running. Physical examination is normal.',
      pt: 'Homem de 48 anos, sem antecedentes, que consulta por «solavancos» no peito que nota sobretudo quando se deita e fica quieto. Não tem dor, nem falta de ar, nem desmaiou. Corre três vezes por semana e diz que a correr não os sente. O exame físico é normal.',
    },
    options: [
      { id: 'extrasistole', label: { es: 'Extrasístole ventricular aislada', en: 'Isolated ventricular premature beat', pt: 'Extrassístole ventricular isolada' } },
      { id: 'auricular', label: { es: 'Extrasístole auricular', en: 'Atrial premature beat', pt: 'Extrassístole atrial' } },
      { id: 'tv', label: { es: 'Taquicardia ventricular no sostenida', en: 'Non-sustained ventricular tachycardia', pt: 'Taquicardia ventricular não sustentada' } },
      { id: 'artefacto', label: { es: 'Artefacto de movimiento', en: 'Movement artefact', pt: 'Artefacto de movimento' } },
    ],
    explain: {
      es: 'En medio de un ritmo sinusal regular a 68 hay un latido que se sale de la fila, y se sale por dos motivos a la vez. Primero, llega ANTES: aparece al 60 % del ciclo, o sea que se adelantó casi la mitad del intervalo. Segundo, no se PARECE a los demás: comparado con la plantilla del latido típico da 92 % de parecido, cuando todos los otros dan 99 o 100. Esas dos cosas juntas son la definición. Y explican de dónde salió. El impulso no nació en la aurícula: si hubiera nacido ahí, habría bajado por el His y las dos ramas, y el complejo habría salido idéntico a los demás, sólo que antes — eso es una extrasístole auricular, que es la primera opción descartada. Acá el complejo es distinto porque el impulso nació EN el ventrículo, fuera del sistema de conducción, y tuvo que propagarse de músculo en músculo: más lento, en otra dirección, con otra forma. Fijate en lo que viene después: una pausa. El latido ectópico dejó al ventrículo refractario cuando llegó el impulso sinusal siguiente, así que ese se perdió, y el ritmo retoma recién con el siguiente. Por eso el paciente siente un vuelco y no un latido de más. Tampoco es una taquicardia ventricular, que necesita tres latidos ventriculares seguidos y acá hay uno solo. Y no es artefacto: un artefacto deforma la línea pero no altera el ritmo, y acá el latido tiene su pausa detrás, o sea que el corazón de verdad hizo algo.',
      en: 'In the middle of a regular sinus rhythm at 68 there is one beat that steps out of line, and it does so for two reasons at once. First, it arrives EARLY: it appears at 60 % of the cycle, nearly half an interval ahead. Second, it does not LOOK like the others: compared with the typical-beat template it scores 92 % similarity, where all the rest score 99 or 100. Those two things together are the definition. And they explain where it came from. The impulse was not born in the atrium: had it been, it would have travelled down the His and both bundles, and the complex would have come out identical to the others, only earlier — that is an atrial premature beat, the first option ruled out. Here the complex is different because the impulse was born IN the ventricle, outside the conduction system, and had to spread muscle to muscle: slower, in another direction, with another shape. Note what follows: a pause. The ectopic beat left the ventricle refractory when the next sinus impulse arrived, so that one was lost, and the rhythm only resumes with the following beat. That is why the patient feels a flip-flop rather than an extra beat. Nor is it ventricular tachycardia, which needs three ventricular beats in a row and here there is only one. And it is not an artefact: an artefact deforms the trace but does not alter the rhythm, and here the beat has its pause behind it, meaning the heart really did something.',
      pt: 'No meio de um ritmo sinusal regular a 68 há um batimento que sai da fila, e sai por dois motivos ao mesmo tempo. Primeiro, chega ANTES: aparece aos 60 % do ciclo, quase meio intervalo adiantado. Segundo, não se PARECE com os outros: comparado com o modelo do batimento típico dá 92 % de semelhança, quando todos os outros dão 99 ou 100. Essas duas coisas juntas são a definição. E explicam de onde saiu. O impulso não nasceu na aurícula: se tivesse nascido aí, teria descido pelo His e pelos dois ramos, e o complexo teria saído idêntico aos outros, só que antes — isso é uma extrassístole atrial, a primeira opção excluída. Aqui o complexo é diferente porque o impulso nasceu NO ventrículo, fora do sistema de condução, e teve de se propagar de músculo em músculo: mais lento, noutra direção, com outra forma. Repare no que vem a seguir: uma pausa. O batimento ectópico deixou o ventrículo refratário quando chegou o impulso sinusal seguinte, e esse perdeu-se; o ritmo só retoma com o seguinte. Por isso o paciente sente um solavanco e não um batimento a mais. Também não é uma taquicardia ventricular, que precisa de três batimentos ventriculares seguidos e aqui há um só. E não é artefacto: um artefacto deforma a linha mas não altera o ritmo, e aqui o batimento tem a sua pausa atrás, ou seja, o coração fez mesmo alguma coisa.',
    },
    pitfall: {
      es: 'La trampa clínica es tratar el síntoma como si fuera el riesgo. Las extrasístoles ventriculares aparecen en casi todo el mundo —con un Holter de 24 horas las tiene más del 90 % de la población sana— y en un corazón estructuralmente normal no acortan la vida ni requieren tratamiento. Lo que hay que averiguar no es cuántas hay sino sobre qué corazón caen: la misma extrasístole es banal en este hombre y es un marcador de riesgo en alguien con un infarto previo o una miocardiopatía. Un detalle del relato que apunta en la buena dirección: las siente en reposo y desaparecen corriendo. Las extrasístoles benignas suelen suprimirse con el ejercicio, porque al acelerar el ritmo sinusal éste «tapa» al foco ectópico; las que EMPEORAN con el esfuerzo son las que preocupan. Y hay una trampa de medición que este mismo trazado deja ver: el QRS general da 112 ms, más ancho de lo que correspondería. No es que el paciente tenga un trastorno de conducción — es que el ancho se calcula sobre un latido promedio y ese promedio incluye la extrasístole, que es ancha por definición. Cuando hay latidos de más de una forma, el promedio deja de representar a ninguno. Conviene saberlo antes de informar un QRS ancho que no existe.',
      en: 'The clinical trap is treating the symptom as if it were the risk. Ventricular premature beats appear in almost everyone — on a 24-hour Holter more than 90 % of healthy people have them — and in a structurally normal heart they neither shorten life nor require treatment. What has to be established is not how many there are but what heart they fall on: the same beat is trivial in this man and a risk marker in someone with a previous infarct or a cardiomyopathy. One detail in the history points the right way: he feels them at rest and they disappear while running. Benign premature beats are usually suppressed by exercise, because as the sinus rate speeds up it "overdrives" the ectopic focus; the ones that get WORSE with effort are the worrying ones. And there is a measurement trap this very tracing reveals: the overall QRS reads 112 ms, wider than it should be. It is not that the patient has a conduction disturbance — it is that the width is computed on an averaged beat and that average includes the premature beat, which is wide by definition. When beats come in more than one shape, the average stops representing any of them. Worth knowing before reporting a wide QRS that does not exist.',
      pt: 'A armadilha clínica é tratar o sintoma como se fosse o risco. As extrassístoles ventriculares aparecem em quase toda a gente — num Holter de 24 horas tem-nas mais de 90 % da população saudável — e num coração estruturalmente normal não encurtam a vida nem requerem tratamento. O que há que averiguar não é quantas há mas sobre que coração caem: a mesma extrassístole é banal neste homem e é um marcador de risco em alguém com um infarto prévio ou uma miocardiopatia. Um detalhe da história que aponta na boa direção: sente-as em repouso e desaparecem a correr. As extrassístoles benignas costumam suprimir-se com o exercício, porque ao acelerar o ritmo sinusal este «tapa» o foco ectópico; as que PIORAM com o esforço são as preocupantes. E há uma armadilha de medição que este mesmo traçado deixa ver: o QRS geral dá 112 ms, mais largo do que corresponderia. Não é que o paciente tenha um distúrbio de condução — é que a largura se calcula sobre um batimento médio e essa média inclui a extrassístole, que é larga por definição. Quando há batimentos de mais de uma forma, a média deixa de representar nenhum. Convém sabê-lo antes de reportar um QRS largo que não existe.',
    },
    action: {
      es: 'Con un corazón presumiblemente normal, síntomas leves y extrasístoles que ceden con el ejercicio, lo que corresponde es tranquilizar y no medicar. Pero «tranquilizar» no es decir que no es nada sin haber mirado: hay un mínimo razonable, y son tres cosas. Un ecocardiograma, para confirmar que la estructura es normal, que es lo que decide el pronóstico. Un análisis con potasio, magnesio y función tiroidea, porque son causas corregibles y frecuentes. Y preguntar por lo obvio, que muchas veces alcanza: café, alcohol, bebidas energéticas, descongestivos, falta de sueño. Si esos tres pasos son normales, la conversación termina ahí y el paciente se va sabiendo por qué. Dos situaciones cambiarían la conducta. Una: si las extrasístoles fueran muy frecuentes —por encima del 10 % de los latidos en un Holter de 24 horas— porque a partir de ahí pueden deteriorar la función del ventrículo con el tiempo, y ahí sí se trata, primero con betabloqueantes y eventualmente con ablación del foco. La otra, y es la que no se negocia: si aparecen síncope, extrasístoles que empeoran con el esfuerzo, antecedentes familiares de muerte súbita, o si el eco muestra cardiopatía, deja de ser un hallazgo benigno y pasa a estudio cardiológico completo.',
      en: 'With a presumably normal heart, mild symptoms and premature beats that settle with exercise, the right course is to reassure and not to medicate. But "reassure" does not mean saying it is nothing without having looked: there is a reasonable minimum, and it is three things. An echocardiogram, to confirm the structure is normal, which is what determines prognosis. Blood tests with potassium, magnesium and thyroid function, because these are correctable and common causes. And asking the obvious, which is often enough: coffee, alcohol, energy drinks, decongestants, lack of sleep. If those three steps are normal, the conversation ends there and the patient leaves knowing why. Two situations would change management. One: if the beats were very frequent — above 10 % of beats on a 24-hour Holter — because beyond that they can impair ventricular function over time, and then treatment is warranted, first with beta blockers and eventually with ablation of the focus. The other, and this one is not negotiable: if syncope appears, if the beats worsen with effort, if there is a family history of sudden death, or if the echo shows heart disease, it stops being a benign finding and goes to full cardiological assessment.',
      pt: 'Com um coração presumivelmente normal, sintomas ligeiros e extrassístoles que cedem com o exercício, o que corresponde é tranquilizar e não medicar. Mas «tranquilizar» não é dizer que não é nada sem ter olhado: há um mínimo razoável, e são três coisas. Um ecocardiograma, para confirmar que a estrutura é normal, que é o que decide o prognóstico. Uma análise com potássio, magnésio e função tiroideia, porque são causas corrigíveis e frequentes. E perguntar o óbvio, que muitas vezes basta: café, álcool, bebidas energéticas, descongestionantes, falta de sono. Se esses três passos forem normais, a conversa termina aí e o paciente sai a saber porquê. Duas situações mudariam a conduta. Uma: se as extrassístoles fossem muito frequentes — acima de 10 % dos batimentos num Holter de 24 horas — porque a partir daí podem deteriorar a função do ventrículo com o tempo, e aí sim trata-se, primeiro com betabloqueadores e eventualmente com ablação do foco. A outra, e essa não se negoceia: se surgirem síncope, extrassístoles que pioram com o esforço, antecedentes familiares de morte súbita, ou se o eco mostrar cardiopatia, deixa de ser um achado benigno e passa a estudo cardiológico completo.',
    },
  },

  {
    ...
    record: '18550',
    age: 67, sex: 'F',
    vitals: { bp: '118/72', spo2: 91, rr: 22 },
    highlight: ['II', 'V1'],
    answer: 'auricula-derecha',
    metrics: { kind: 'p' },
    // Medido: onda P de 2,7 mm en II sobre el latido promedio. FC 86, QRS
    // 104 ms, eje +43°, PR 140 ms, cv 0,006, ruido 30 µV. El ST de los miembros
    // no pasa de 20 µV en ninguna derivación.
    findings: {
      // El umbral clásico, y comprobado contra la base: los normales dan 1,0 mm
      // de mediana y ninguno de 25 llega a 2,5; los etiquetados con crecimiento
      // derecho dan 2,4.
      pHeight: { min: 0.25, lead: 'II' },
      qrsMs: [80, 115],
      rate: [72, 98],
      irregular: false,
      stFlat: { leads: ['I', 'II', 'III', 'aVR', 'aVL', 'aVF'], max: 0.05 },
    },
    stem: {
      es: 'Mujer de 67 años, fumadora de toda la vida, que consulta por falta de aire que empeoró en los últimos meses. Tose por las mañanas desde hace años. Está saturando 91 % al aire ambiente y respira 22 por minuto. Se le ven las venas del cuello llenas y tiene los tobillos hinchados.',
      en: '67-year-old woman, a lifelong smoker, seen for breathlessness that has worsened over recent months. She has coughed in the mornings for years. She is saturating 91 % on room air and breathing 22 per minute. Her neck veins are full and her ankles are swollen.',
      pt: 'Mulher de 67 anos, fumadora de toda a vida, que consulta por falta de ar que piorou nos últimos meses. Tosse de manhã há anos. Está a saturar 91 % em ar ambiente e respira 22 por minuto. Vêem-se-lhe as veias do pescoço cheias e tem os tornozelos inchados.',
    },
    options: [
      { id: 'auricula-derecha', label: { es: 'Sobrecarga de la aurícula derecha', en: 'Right atrial overload', pt: 'Sobrecarga da aurícula direita' } },
      { id: 'auricula-izquierda', label: { es: 'Sobrecarga de la aurícula izquierda', en: 'Left atrial overload', pt: 'Sobrecarga da aurícula esquerda' } },
      { id: 'nodal', label: { es: 'Ritmo auricular ectópico', en: 'Ectopic atrial rhythm', pt: 'Ritmo atrial ectópico' } },
      { id: 'normal', label: { es: 'Onda P normal', en: 'Normal P wave', pt: 'Onda P normal' } },
    ],
    explain: {
      es: 'Todo el caso está en la primera onda de cada latido, que es la que casi nadie mide. La P de II mide 2,7 mm de alto. El techo normal son 2,5, y para dimensionarlo: sobre los registros normales de esta misma base la mediana es 1,0 mm y ninguno de veinticinco llega a 2,5. Ésta casi la triplica. La P es la suma de dos despolarizaciones que ocurren casi a la vez: primero la aurícula derecha, después la izquierda. Cuando la derecha se agranda, su parte de la onda crece en ALTURA —porque el vector se hace más grande apuntando hacia abajo y a la derecha, justo hacia donde mira II— y como sigue empezando y terminando antes que la izquierda, la P no se alarga: se pone alta y puntiaguda. Eso es la P «pulmonale», y el nombre dice de dónde viene. La izquierda funciona al revés, y por eso la segunda opción se descarta midiendo otra cosa: cuando se agranda la aurícula izquierda su despolarización, que es la que termina más tarde, se retrasa todavía más, y la P se pone ANCHA y con dos jorobas, no alta. Tampoco es un ritmo auricular ectópico: en ése la P cambia de FORMA o de polaridad —puede salir negativa en II— y acá es positiva, normal de forma, sólo que grande. Y no es normal, porque 2,7 mm no lo es.',
      en: 'The whole case is in the first wave of each beat, the one almost nobody measures. The P in II is 2.7 mm tall. The normal ceiling is 2.5, and to put it in scale: in the normal records of this same database the median is 1.0 mm and not one of twenty-five reaches 2.5. This one nearly triples it. The P is the sum of two depolarizations that happen almost together: first the right atrium, then the left. When the right one enlarges, its part of the wave grows in HEIGHT — because the vector gets bigger pointing down and to the right, exactly where II is looking — and since it still starts and finishes before the left, the P does not lengthen: it becomes tall and peaked. That is the "P pulmonale", and the name says where it comes from. The left works the other way round, which is how the second option is ruled out by measuring something else: when the left atrium enlarges, its depolarization, the one that finishes later, is delayed further still, and the P becomes WIDE and double-humped, not tall. Nor is it an ectopic atrial rhythm: there the P changes SHAPE or polarity — it may come out negative in II — and here it is upright, normal in shape, just large. And it is not normal, because 2.7 mm is not.',
      pt: 'Todo o caso está na primeira onda de cada batimento, a que quase ninguém mede. A P de II mede 2,7 mm de altura. O teto normal são 2,5, e para dimensionar: nos registros normais desta mesma base a mediana é 1,0 mm e nenhum de vinte e cinco chega a 2,5. Esta quase a triplica. A P é a soma de duas despolarizações que acontecem quase ao mesmo tempo: primeiro a aurícula direita, depois a esquerda. Quando a direita aumenta, a sua parte da onda cresce em ALTURA — porque o vetor fica maior a apontar para baixo e para a direita, justamente para onde II olha — e como continua a começar e a terminar antes da esquerda, a P não se alonga: fica alta e pontiaguda. Isso é a P «pulmonale», e o nome diz de onde vem. A esquerda funciona ao contrário, e é assim que a segunda opção se exclui medindo outra coisa: quando a aurícula esquerda aumenta, a sua despolarização, a que termina mais tarde, atrasa-se ainda mais, e a P fica LARGA e com duas corcovas, não alta. Também não é um ritmo atrial ectópico: nesse a P muda de FORMA ou de polaridade — pode sair negativa em II — e aqui é positiva, normal de forma, só que grande. E não é normal, porque 2,7 mm não o é.',
    },
    pitfall: {
      es: 'El error más común con la onda P es no mirarla. Es la primera de cada latido y la más chica, y la vista salta directo al QRS y al ST, que es donde uno espera encontrar algo. Conviene tener una rutina fija: antes de nada, medir la P en II, de alto y de ancho. Dos números y se acabó. El segundo error es de vocabulario, y esconde una limitación real: «crecimiento auricular» sugiere que el electro está midiendo el tamaño de la aurícula, y no es así. Lo que la P alta refleja es la SOBRECARGA —presión o volumen— del lado derecho, y puede aparecer y desaparecer con el estado del paciente: una P pulmonale puede normalizarse cuando se compensa una reagudización del EPOC, sin que la aurícula haya cambiado de tamaño. Por eso conviene informarlo como «sobrecarga» y no como «crecimiento», y por eso el propio cardiólogo que anotó este registro escribió «PROBABLE right atrial enlargement». Tercer punto, sobre este trazado en particular: hay descenso del ST en las precordiales y aplanamiento de la T, que el informe llama inespecíficos y que no son el hallazgo del caso. Conviene no dejarlos sin mencionar, pero tampoco construir el diagnóstico sobre ellos. Y una cosa que este electro NO dice: nada sobre la aurícula izquierda ni sobre el ventrículo. La P alta habla del lado derecho y de nada más.',
      en: 'The commonest mistake with the P wave is not looking at it. It is the first of each beat and the smallest, and the eye jumps straight to the QRS and the ST, where one expects to find something. A fixed routine helps: before anything else, measure the P in II, height and width. Two numbers and done. The second mistake is one of vocabulary, and it hides a real limitation: "atrial enlargement" suggests the ECG is measuring the size of the atrium, and it is not. What a tall P reflects is OVERLOAD — pressure or volume — of the right side, and it can come and go with the patient\u2019s state: a P pulmonale may normalize when a COPD exacerbation is compensated, without the atrium having changed size at all. That is why it is better reported as "overload" than as "enlargement", and it is why the cardiologist who annotated this record wrote "PROBABLE right atrial enlargement". Third point, about this tracing in particular: there is ST depression in the chest leads and T wave flattening, which the report calls non-specific and which are not the finding of this case. Worth mentioning, but not worth building the diagnosis on. And something this ECG does NOT say: anything about the left atrium or the ventricle. A tall P speaks of the right side and of nothing else.',
      pt: 'O erro mais comum com a onda P é não a olhar. É a primeira de cada batimento e a mais pequena, e a vista salta direto para o QRS e para o ST, que é onde se espera encontrar algo. Convém ter uma rotina fixa: antes de tudo, medir a P em II, de alto e de largo. Dois números e pronto. O segundo erro é de vocabulário, e esconde uma limitação real: «crescimento auricular» sugere que o ECG está a medir o tamanho da aurícula, e não é assim. O que a P alta reflete é a SOBRECARGA — pressão ou volume — do lado direito, e pode aparecer e desaparecer com o estado do paciente: uma P pulmonale pode normalizar quando se compensa uma reagudização da DPOC, sem que a aurícula tenha mudado de tamanho. Por isso convém reportá-lo como «sobrecarga» e não como «crescimento», e por isso o próprio cardiologista que anotou este registro escreveu «PROVÁVEL right atrial enlargement». Terceiro ponto, sobre este traçado em particular: há descenso do ST nas precordiais e achatamento da T, que o laudo chama inespecíficos e que não são o achado do caso. Convém mencioná-los, mas não construir o diagnóstico sobre eles. E algo que este ECG NÃO diz: nada sobre a aurícula esquerda nem sobre o ventrículo. A P alta fala do lado direito e de mais nada.',
    },
    action: {
      es: 'El electro no es el diagnóstico: es la pista que ordena el resto. Una P pulmonale en una fumadora de toda la vida con disnea, saturación de 91 %, ingurgitación yugular y edemas apunta a una sola historia —enfermedad pulmonar crónica que terminó sobrecargando el lado derecho— y lo que hay que hacer es confirmarla y medirla. El estudio que más aporta es el ecocardiograma, y hay que pedirlo mirando el lado derecho específicamente: tamaño y función del ventrículo derecho, tamaño de la aurícula derecha, y estimación de la presión pulmonar por el chorro de insuficiencia tricuspídea. En paralelo, una espirometría para poner número a la obstrucción, que es lo que va a guiar el tratamiento de fondo, y una gasometría arterial, porque la saturación de 91 % en reposo obliga a saber si hay hipercapnia y si corresponde oxígeno domiciliario, que es de las pocas cosas que cambian la supervivencia en el EPOC. Una radiografía de tórax también, que es barata y ordena bastante. Y lo primero de todo, que no es un estudio: dejar de fumar. Es la única intervención que modifica la evolución de la enfermedad de base, y conviene decirlo en la misma consulta en que se explica el electro, porque es cuando el paciente está escuchando.',
      en: 'The ECG is not the diagnosis: it is the clue that organizes the rest. A P pulmonale in a lifelong smoker with breathlessness, 91 % saturation, raised neck veins and oedema points to a single story — chronic lung disease that ended up overloading the right side — and what has to be done is to confirm and quantify it. The most useful test is the echocardiogram, and it should be requested looking at the right side specifically: right ventricular size and function, right atrial size, and estimation of pulmonary pressure from the tricuspid regurgitation jet. In parallel, spirometry to put a number on the obstruction, which will guide the underlying treatment, and an arterial blood gas, because 91 % saturation at rest makes it necessary to know whether there is hypercapnia and whether home oxygen is indicated, which is one of the few things that changes survival in COPD. A chest X-ray as well, which is cheap and clarifies a lot. And first of all, which is not a test: stopping smoking. It is the only intervention that modifies the course of the underlying disease, and it is worth saying in the same consultation where the ECG is explained, because that is when the patient is listening.',
      pt: 'O ECG não é o diagnóstico: é a pista que organiza o resto. Uma P pulmonale numa fumadora de toda a vida com dispneia, saturação de 91 %, ingurgitação jugular e edemas aponta para uma só história — doença pulmonar crónica que acabou por sobrecarregar o lado direito — e o que há que fazer é confirmá-la e medi-la. O exame que mais aporta é o ecocardiograma, e há que pedi-lo a olhar o lado direito especificamente: tamanho e função do ventrículo direito, tamanho da aurícula direita, e estimativa da pressão pulmonar pelo jato de insuficiência tricúspide. Em paralelo, uma espirometria para pôr número na obstrução, que é o que vai guiar o tratamento de fundo, e uma gasometria arterial, porque a saturação de 91 % em repouso obriga a saber se há hipercapnia e se corresponde oxigénio domiciliário, que é das poucas coisas que mudam a sobrevivência na DPOC. Uma radiografia de tórax também, que é barata e organiza bastante. E antes de tudo, que não é um exame: deixar de fumar. É a única intervenção que modifica a evolução da doença de base, e convém dizê-lo na mesma consulta em que se explica o ECG, porque é quando o paciente está a ouvir.',
    },
  },

  {
    ...
    record: 'JS12422',
    age: 61, sex: 'M',
    vitals: { bp: '122/76', spo2: 97, rr: 16 },
    highlight: ['II', 'aVF'],
    answer: 'wenckebach',
    metrics: { kind: 'wenckebach' },
    // Medido: el PR latido a latido da 356 · 228 · 360 · 232 · 360 · 356 · —
    // · 364 · 364 · — ms, y se estira 228 ms de punta a punta. Los RR alternan
    // 616 y 1104 ms: conducción 3:2, dos latidos conducidos y uno bloqueado.
    // La pausa entra 1,83 veces en el RR corto —menos de dos—, que es la firma
    // del decremento. FC 97, QRS 88 ms, ruido 60 µV.
    //
    // La serie coincide en cinco derivaciones dentro de 25 ms: si fuera ruido
    // no coincidirían. Ése es el control que faltó en el primer intento.
    findings: {
      // En ritmo sinusal normal esta cifra ronda los 20 ms. Acá, 228.
      prLengthening: { min: 150 },
      // Y la pausa SIN llegar a dos ciclos, que es lo que lo separa de una
      // extrasístole auricular bloqueada (caso 24, razón 2,21).
      pauseRatio: [1.5, 1.99],
      qrsMs: [70, 110],
    },
    stem: {
      es: 'Hombre de 61 años en control por hipertensión. Toma un betabloqueante desde hace tres meses. No tiene síntomas: ni mareos, ni cansancio, ni desmayos. Le hacen este electro de rutina y el informe automático dice «bloqueo AV de segundo grado».',
      en: '61-year-old man at a hypertension follow-up. He has been on a beta blocker for three months. He has no symptoms: no dizziness, no fatigue, no blackouts. This routine ECG is recorded and the machine report reads "second degree AV block".',
      pt: 'Homem de 61 anos em consulta de hipertensão. Toma um betabloqueador há três meses. Não tem sintomas: nem tonturas, nem cansaço, nem desmaios. Fazem-lhe este ECG de rotina e o laudo automático diz «bloqueio AV de segundo grau».',
    },
    options: [
      { id: 'wenckebach', label: { es: 'Bloqueo AV de segundo grado, Mobitz I (Wenckebach)', en: 'Second degree AV block, Mobitz I (Wenckebach)', pt: 'Bloqueio AV de segundo grau, Mobitz I (Wenckebach)' } },
      { id: 'mobitz2', label: { es: 'Bloqueo AV de segundo grado, Mobitz II', en: 'Second degree AV block, Mobitz II', pt: 'Bloqueio AV de segundo grau, Mobitz II' } },
      { id: 'extrasistole', label: { es: 'Extrasístole auricular bloqueada', en: 'Blocked atrial premature beat', pt: 'Extrassístole atrial bloqueada' } },
      { id: 'bigeminismo', label: { es: 'Bigeminismo ventricular', en: 'Ventricular bigeminy', pt: 'Bigeminismo ventricular' } },
    ],
    explain: {
      es: 'Lo primero que salta es el AGRUPAMIENTO: los latidos vienen de a pares, y entre par y par hay un hueco. Eso ya es una pista —cuando los latidos se agrupan, hay que contar—. Los RR miden 616 y 1104 alternándose: corto, largo, corto, largo. Y ahora lo que lo define, que está en el PR de cada latido por separado. La serie mide 356, 228, 360, 232, 360… El primer latido de cada par conduce con un PR de unos 230 ms; el segundo, con unos 360. Y entonces la tercera P no conduce, y aparece el hueco. Después el ciclo vuelve a empezar desde 230. Eso es Wenckebach: el nodo AV se va cansando latido a latido hasta que una P no pasa, y la pausa le da tiempo de recuperarse. Acá el ciclo es 3:2 —tres P, dos conducidas—. El PR se estira 228 ms de punta a punta; en un ritmo sinusal normal esa cifra ronda los 20. Y hay un segundo número que confirma la historia sin necesidad de ver una sola P: la pausa entra 1,83 veces en el RR corto, o sea MENOS de dos. Eso sólo pasa si hubo decremento antes: como el segundo PR ya venía largo, el latido que lo precede queda «empujado» y la pausa resulta más corta que dos ciclos enteros.',
      en: 'The first thing that jumps out is the GROUPING: the beats come in pairs, with a gap between one pair and the next. That is already a clue — when beats cluster, start counting. The RR intervals measure 616 and 1104 alternating: short, long, short, long. And now what defines it, which is in the PR of each individual beat. The series reads 356, 228, 360, 232, 360… The first beat of each pair conducts with a PR of about 230 ms; the second, with about 360. And then the third P fails to conduct, and the gap appears. Then the cycle starts again from 230. That is Wenckebach: the AV node tires beat by beat until one P does not get through, and the pause gives it time to recover. Here the cycle is 3:2 — three P waves, two conducted. The PR stretches 228 ms end to end; in normal sinus rhythm that figure is around 20. And there is a second number that confirms the story without seeing a single P: the pause fits 1.83 times into the short RR, that is LESS than twice. That only happens if there was decrement beforehand: because the second PR was already long, the beat preceding it is "pushed along" and the pause comes out shorter than two whole cycles.',
      pt: 'A primeira coisa que salta é o AGRUPAMENTO: os batimentos vêm aos pares, e entre par e par há um buraco. Isso já é uma pista — quando os batimentos se agrupam, há que contar. Os RR medem 616 e 1104 alternadamente: curto, longo, curto, longo. E agora o que o define, que está no PR de cada batimento em separado. A série mede 356, 228, 360, 232, 360… O primeiro batimento de cada par conduz com um PR de cerca de 230 ms; o segundo, com cerca de 360. E então a terceira P não conduz, e aparece o buraco. Depois o ciclo recomeça a partir de 230. Isso é Wenckebach: o nó AV vai-se cansando batimento a batimento até que uma P não passa, e a pausa dá-lhe tempo de se recuperar. Aqui o ciclo é 3:2 — três P, duas conduzidas. O PR estica-se 228 ms de ponta a ponta; num ritmo sinusal normal esse valor ronda os 20. E há um segundo número que confirma a história sem ver uma única P: a pausa cabe 1,83 vezes no RR curto, ou seja, MENOS de duas. Isso só acontece se houve decremento antes: como o segundo PR já vinha longo, o batimento que o precede fica «empurrado» e a pausa resulta mais curta do que dois ciclos inteiros.',
    },
    pitfall: {
      es: 'La distinción que importa es con el Mobitz II, porque uno es benigno y el otro termina en marcapasos. En el Mobitz II el PR es CONSTANTE y de golpe una P no conduce: no hay decremento, no hay aviso, y el bloqueo está por debajo del nodo AV, en el His-Purkinje, que es tejido que no se recupera y progresa a bloqueo completo. En el Wenckebach el PR se estira antes de fallar, el bloqueo está EN el nodo AV, responde a la atropina y al ejercicio, y casi nunca progresa. Los dos números de este electro dicen Mobitz I: el PR se estira 228 ms, y la pausa mide menos de dos ciclos. En un Mobitz II la pausa mediría casi exactamente dos, porque la P se bloquea sin que nada se haya estirado antes. La segunda confusión es con la extrasístole auricular bloqueada, que es el caso 24 de esta sección y también da una pausa. Ahí la razón daba 2,21 —más de dos— y los PR conducidos eran todos iguales: el nodo AV estaba sano y lo que falló fue una P que llegó demasiado pronto. Acá es al revés. Y una advertencia honesta sobre este trazado en particular: es de bajo voltaje, y las ondas P se ven mal a simple vista. Por eso el caso se apoya en las huellas —el agrupamiento y la razón de la pausa—, que es exactamente como se enseña a leer un Wenckebach cuando las P no se distinguen. Los números del panel son los que el ojo no puede sacar de este papel, y están ahí para que se vea qué hay que buscar cuando el trazado sí lo permita.',
      en: 'The distinction that matters is from Mobitz II, because one is benign and the other ends in a pacemaker. In Mobitz II the PR is CONSTANT and suddenly a P does not conduct: no decrement, no warning, and the block sits below the AV node, in the His-Purkinje system, which is tissue that does not recover and does progress to complete block. In Wenckebach the PR stretches before failing, the block is IN the AV node, it responds to atropine and to exercise, and it almost never progresses. Both numbers on this ECG say Mobitz I: the PR stretches 228 ms, and the pause measures less than two cycles. In a Mobitz II the pause would measure almost exactly two, because the P is blocked without anything having stretched beforehand. The second confusion is with the blocked atrial premature beat, which is case 24 of this section and also produces a pause. There the ratio was 2.21 — more than two — and the conducted PR intervals were all equal: the AV node was healthy and what failed was a P that arrived too early. Here it is the other way round. And an honest warning about this tracing in particular: it is low voltage, and the P waves are hard to see by eye. That is why the case leans on the footprints — the grouping and the pause ratio — which is exactly how one is taught to read a Wenckebach when the P waves cannot be made out. The numbers in the panel are the ones the eye cannot get from this paper, and they are there so you know what to look for when the tracing does allow it.',
      pt: 'A distinção que importa é com o Mobitz II, porque um é benigno e o outro acaba em marca-passo. No Mobitz II o PR é CONSTANTE e de repente uma P não conduz: não há decremento, não há aviso, e o bloqueio fica abaixo do nó AV, no His-Purkinje, que é tecido que não se recupera e progride para bloqueio completo. No Wenckebach o PR estica-se antes de falhar, o bloqueio está NO nó AV, responde à atropina e ao exercício, e quase nunca progride. Os dois números deste ECG dizem Mobitz I: o PR estica-se 228 ms, e a pausa mede menos de dois ciclos. Num Mobitz II a pausa mediria quase exatamente dois, porque a P bloqueia-se sem que nada se tenha esticado antes. A segunda confusão é com a extrassístole atrial bloqueada, que é o caso 24 desta seção e também produz uma pausa. Aí a razão dava 2,21 — mais de dois — e os PR conduzidos eram todos iguais: o nó AV estava saudável e o que falhou foi uma P que chegou cedo demais. Aqui é ao contrário. E um aviso honesto sobre este traçado em particular: é de baixa voltagem, e as ondas P veem-se mal a olho nu. Por isso o caso apoia-se nas pegadas — o agrupamento e a razão da pausa —, que é exatamente como se ensina a ler um Wenckebach quando as P não se distinguem. Os números do painel são os que o olho não consegue tirar deste papel, e estão ali para que se saiba o que procurar quando o traçado o permitir.',
    },
    action: {
      es: 'Lo primero es la lista de medicamentos, y en este caso está escrita en el enunciado: empezó un betabloqueante hace tres meses. Un Wenckebach que aparece después de introducir un frenador del nodo AV —betabloqueantes, verapamilo, diltiazem, digoxina, amiodarona— es, hasta que se demuestre lo contrario, efecto del fármaco, y la conducta es ajustar la dosis o cambiar de familia y repetir el electro, no poner un marcapasos. En alguien SIN síntomas y con QRS angosto, como acá, eso suele alcanzar. Vale saber además que el Wenckebach es un hallazgo normal durante el sueño y en deportistas entrenados, por tono vagal alto: si aparece sólo de noche en un Holter y desaparece con el ejercicio, no es enfermedad. Y hay una maniobra diagnóstica barata que confirma el nivel del bloqueo: hacer caminar o subir escaleras al paciente. El Wenckebach nodal MEJORA con el ejercicio —el simpático facilita el nodo AV— mientras que el Mobitz II empeora, porque la frecuencia auricular sube y el His-Purkinje enfermo no la sigue. Lo que sí cambia todo: si hubiera síncope, mareo, bradicardia sintomática, o si el QRS fuera ancho —que sugiere enfermedad distal—, deja de ser un hallazgo benigno y va a cardiología, con marcapasos sobre la mesa.',
      en: 'The first thing is the medication list, and in this case it is written into the vignette: he started a beta blocker three months ago. A Wenckebach that appears after introducing an AV nodal blocker — beta blockers, verapamil, diltiazem, digoxin, amiodarone — is, until proven otherwise, a drug effect, and the management is to adjust the dose or change class and repeat the ECG, not to implant a pacemaker. In someone WITHOUT symptoms and with a narrow QRS, as here, that usually suffices. It is also worth knowing that Wenckebach is a normal finding during sleep and in trained athletes, from high vagal tone: if it appears only at night on a Holter and disappears with exercise, it is not disease. And there is a cheap diagnostic manoeuvre that confirms the level of the block: have the patient walk or climb stairs. Nodal Wenckebach IMPROVES with exercise — sympathetic drive facilitates the AV node — whereas Mobitz II worsens, because the atrial rate rises and the diseased His-Purkinje cannot follow. What does change everything: if there were syncope, dizziness, symptomatic bradycardia, or if the QRS were wide — suggesting distal disease — it stops being a benign finding and goes to cardiology, with a pacemaker on the table.',
      pt: 'O primeiro é a lista de medicamentos, e neste caso está escrita no enunciado: começou um betabloqueador há três meses. Um Wenckebach que aparece depois de introduzir um travão do nó AV — betabloqueadores, verapamil, diltiazem, digoxina, amiodarona — é, até prova em contrário, efeito do fármaco, e a conduta é ajustar a dose ou mudar de família e repetir o ECG, não pôr um marca-passo. Em alguém SEM sintomas e com QRS estreito, como aqui, isso costuma bastar. Vale saber ainda que o Wenckebach é um achado normal durante o sono e em atletas treinados, por tónus vagal alto: se aparece só de noite num Holter e desaparece com o exercício, não é doença. E há uma manobra diagnóstica barata que confirma o nível do bloqueio: pôr o paciente a caminhar ou a subir escadas. O Wenckebach nodal MELHORA com o exercício — o simpático facilita o nó AV — enquanto o Mobitz II piora, porque a frequência atrial sobe e o His-Purkinje doente não a acompanha. O que muda tudo: se houvesse síncope, tontura, bradicardia sintomática, ou se o QRS fosse largo — sugerindo doença distal —, deixa de ser um achado benigno e vai para cardiologia, com marca-passo em cima da mesa.',
    },
  },

  {
    ...
    record: 'JS22294',
    age: 24, sex: 'M',
    vitals: { bp: '112/68', spo2: 99, rr: 14 },
    highlight: ['V4', 'V5', 'V6'],
    answer: 'repolarizacion',
    metrics: { kind: 'jwave', leads: ['V4', 'V5', 'V6'] },
    // Medido: el punto J sube 8,0 · 6,5 · 4,8 mm en V4, V5 y V6, y esa subida
    // tiene forma de joroba —muesca de 5,7 · 3,4 · 1,1 mm—, o sea que baja de
    // nuevo. El ST medido 60 ms más adelante ya vale 1,2 · 0,7 · 1,5 mm: el
    // segmento volvió casi a la línea. Esa diferencia entre el punto J y el ST
    // sesenta milisegundos después es todo el caso.
    //
    // Alrededor: FC 48, cv 0,016, QRS 96 ms, PR 144, QTc 403, eje +68°. aVL con
    // el ST en −0,0 mm, o sea sin imagen recíproca. Ruido 40 µV.
    //
    // El Sokolow da 36,5 mm, por encima del umbral de hipertrofia, y la base lo
    // etiqueta como tal. En un varón de 24 años delgado eso es voltaje alto
    // normal, y el caso lo dice en vez de esconderlo.
    findings: {
      jPoint: { leads: ['V4', 'V5', 'V6'], min: 0.20 },
      jNotch: { leads: ['V4', 'V5'], min: 0.10 },
      qrsMs: [70, 110],
      stFlat: { leads: ['aVL'], max: 0.10 },
      tUpright: { leads: ['V4', 'V5', 'V6'], min: 0.30 },
      rate: [40, 60],
      irregular: false,
    },
    stem: {
      es: 'Hombre de 24 años, jugador de rugby, que llega a la guardia por una molestia en el pecho que empezó dos horas después de un entrenamiento fuerte. La describe como una presión leve, que cambia cuando respira hondo y que ya casi se le fue. No fuma, no toma nada, no tiene antecedentes ni familiares con muerte súbita. El examen es normal y la primera troponina es negativa. El médico que lo recibe mira el electro, ve el ST alto en las precordiales y llama a hemodinamia.',
      en: '24-year-old man, a rugby player, who arrives at the emergency department with chest discomfort that began two hours after a hard training session. He describes it as a mild pressure, which changes when he breathes deeply and has almost gone. He does not smoke, takes nothing, has no history and no family history of sudden death. Examination is normal and the first troponin is negative. The doctor who receives him looks at the ECG, sees the ST high in the chest leads, and calls the cath lab.',
      pt: 'Homem de 24 anos, jogador de râguebi, que chega à urgência por um desconforto no peito que começou duas horas depois de um treino forte. Descreve-o como uma pressão ligeira, que muda quando respira fundo e que já quase passou. Não fuma, não toma nada, não tem antecedentes nem familiares com morte súbita. O exame é normal e a primeira troponina é negativa. O médico que o recebe olha para o eletro, vê o ST alto nas precordiais e chama a hemodinâmica.',
    },
    options: [
      { id: 'repolarizacion', label: { es: 'Repolarización precoz (onda J)', en: 'Early repolarization (J wave)', pt: 'Repolarização precoce (onda J)' } },
      { id: 'stemi', label: { es: 'Infarto agudo anterolateral', en: 'Acute anterolateral infarction', pt: 'Enfarte agudo anterolateral' } },
      { id: 'pericarditis', label: { es: 'Pericarditis aguda', en: 'Acute pericarditis', pt: 'Pericardite aguda' } },
      { id: 'brugada', label: { es: 'Patrón de Brugada', en: 'Brugada pattern', pt: 'Padrão de Brugada' } },
    ],
    explain: {
      es: 'El punto J está alto: 8,0 mm en V4, 6,5 en V5, 4,8 en V6. Si uno se queda ahí, es un infarto anterolateral y el llamado a hemodinamia está bien hecho. La pregunta que decide el caso es otra: ¿qué hace el segmento ST DESPUÉS de ese punto J? Medido 60 ms más adelante, el ST vale 1,2 mm en V4, 0,7 en V5 y 1,5 en V6. Se cayó. De 8 mm a 1,2 mm en sesenta milisegundos. Eso no es un segmento elevado, es una JOROBA: el QRS, en vez de terminar bajando limpio hasta la línea del ST, se frena justo en el final, hace una muesca de 5,7 mm en V4 —3,4 en V5, 1,1 en V6— y recién después desciende. Esa joroba es la onda J, y es el hallazgo que define la repolarización precoz desde el consenso de 2015. En un infarto agudo el ST sale del punto J y se QUEDA arriba: las dos cifras, la del punto J y la del ST 60 ms después, se parecen. Acá difieren por un factor de siete. El resto del trazado acompaña y hay que mirarlo, porque un solo hallazgo nunca alcanza. No hay imagen recíproca: aVL mide −0,0 mm de ST, y un infarto anterolateral de esta magnitud tendría que descender ahí. Las ondas T son altas y positivas —4,8, 6,2 y 5,3 mm en V4, V5 y V6— y salen de la joroba, no de una meseta. La frecuencia es 48 con arritmia sinusal, y el QRS mide 96 ms. No es pericarditis: ahí el supradesnivel es difuso, alcanza también las derivaciones inferiores y de miembros, y viene con descenso del PR; acá II, III y aVF tienen el ST en 0,9, 0,5 y 0,7 mm. Y no es Brugada: ese patrón vive en V1 y V2 con una R\u2019 ancha y el ST descendiendo en bóveda hacia una T negativa, y en V1 este electro tiene el punto J en 0,8 mm y la T positiva.',
      en: 'The J point is high: 8.0 mm in V4, 6.5 in V5, 4.8 in V6. If you stop there, this is an anterolateral infarct and calling the cath lab is the right move. The question that decides the case is a different one: what does the ST segment do AFTER that J point? Measured 60 ms later, the ST reads 1.2 mm in V4, 0.7 in V5 and 1.5 in V6. It fell. From 8 mm to 1.2 mm in sixty milliseconds. That is not an elevated segment, it is a HUMP: the QRS, instead of ending by descending cleanly to the ST line, slows right at the end, makes a notch of 5.7 mm in V4 — 3.4 in V5, 1.1 in V6 — and only then descends. That hump is the J wave, and it is the finding that has defined early repolarization since the 2015 consensus. In an acute infarct the ST leaves the J point and STAYS up: the two figures, the J point and the ST 60 ms later, resemble each other. Here they differ by a factor of seven. The rest of the tracing agrees, and it has to be looked at, because a single finding is never enough. There is no reciprocal change: aVL measures −0.0 mm of ST, and an anterolateral infarct of this size would have to depress there. The T waves are tall and upright — 4.8, 6.2 and 5.3 mm in V4, V5 and V6 — and they come out of the hump, not off a plateau. The rate is 48 with sinus arrhythmia, and the QRS measures 96 ms. It is not pericarditis: there the elevation is diffuse, reaching the inferior and limb leads too, and comes with PR depression; here II, III and aVF have the ST at 0.9, 0.5 and 0.7 mm. And it is not Brugada: that pattern lives in V1 and V2 with a broad R\u2019 and the ST descending in a coved shape towards a negative T, and in V1 this ECG has the J point at 0.8 mm and the T upright.',
      pt: 'O ponto J está alto: 8,0 mm em V4, 6,5 em V5, 4,8 em V6. Se ficarmos por aí, é um enfarte anterolateral e a chamada à hemodinâmica está bem feita. A pergunta que decide o caso é outra: o que faz o segmento ST DEPOIS desse ponto J? Medido 60 ms adiante, o ST vale 1,2 mm em V4, 0,7 em V5 e 1,5 em V6. Caiu. De 8 mm para 1,2 mm em sessenta milissegundos. Isso não é um segmento elevado, é uma CORCOVA: o QRS, em vez de terminar a descer limpo até à linha do ST, trava mesmo no fim, faz um entalhe de 5,7 mm em V4 — 3,4 em V5, 1,1 em V6 — e só depois desce. Essa corcova é a onda J, e é o achado que define a repolarização precoce desde o consenso de 2015. Num enfarte agudo o ST sai do ponto J e FICA em cima: os dois valores, o do ponto J e o do ST 60 ms depois, parecem-se. Aqui diferem por um fator de sete. O resto do traçado acompanha e há que olhá-lo, porque um só achado nunca chega. Não há imagem recíproca: aVL mede −0,0 mm de ST, e um enfarte anterolateral desta magnitude teria de descer aí. As ondas T são altas e positivas — 4,8, 6,2 e 5,3 mm em V4, V5 e V6 — e saem da corcova, não de um planalto. A frequência é 48 com arritmia sinusal, e o QRS mede 96 ms. Não é pericardite: aí o supradesnivelamento é difuso, alcança também as derivações inferiores e dos membros, e vem com descida do PR; aqui II, III e aVF têm o ST em 0,9, 0,5 e 0,7 mm. E não é Brugada: esse padrão vive em V1 e V2 com uma R\u2019 larga e o ST a descer em abóbada até uma T negativa, e em V1 este eletro tem o ponto J em 0,8 mm e a T positiva.',
    },
    pitfall: {
      es: 'La trampa que más cuesta es la de la palabra «precoz». Suena a hallazgo de gente joven y sana, y durante décadas se enseñó como una variante normal sin más. Desde 2008 se sabe que no es tan simple: la onda J en las derivaciones INFERIORES, cuando es grande y cuando el ST que la sigue es horizontal o descendente en vez de ascendente, se asocia a más riesgo de fibrilación ventricular idiopática. La distinción práctica no es «tiene o no tiene onda J» sino DÓNDE está y QUÉ hace el ST después. Este paciente tiene el hallazgo en las laterales con el ST ascendiendo hacia una T alta, que es la forma de bajo riesgo y de lejos la más frecuente. La segunda trampa es la del contexto, y vale para cualquier electro: esto no se diagnostica desde el papel. Un paciente de 24 años con dolor que cambia con la respiración, troponina negativa y este trazado es una cosa; el mismo trazado en alguien con dolor típico, o después de un síncope de esfuerzo, o con antecedente familiar de muerte súbita, obliga a pensar de nuevo. Y una trampa de medición que este mismo registro deja ver: la base lo etiqueta con hipertrofia ventricular izquierda, y el Sokolow da 36,5 mm, por encima del umbral. Pero es un varón de 24 años, delgado y deportista: la pared torácica fina acerca los electrodos al corazón y sube el voltaje sin que haya un gramo de músculo de más. Los criterios de voltaje tienen mala especificidad justamente en esta población. Lo llamativo del caso —onda J, voltaje alto, bradicardia a 48— es el conjunto que se ve en el corazón de atleta, y los tres hallazgos aparecen juntos por la misma razón.',
      en: 'The hardest trap is the word "early". It sounds like a finding of young healthy people, and for decades it was taught as a normal variant and nothing more. Since 2008 we know it is not that simple: the J wave in the INFERIOR leads, when it is large and when the ST that follows is horizontal or descending rather than ascending, is associated with a higher risk of idiopathic ventricular fibrillation. The practical distinction is not "has or has not a J wave" but WHERE it is and WHAT the ST does afterwards. This patient has the finding in the lateral leads with the ST ascending towards a tall T, which is the low-risk form and by far the commonest. The second trap is context, and it holds for any ECG: this is not diagnosed from the paper. A 24-year-old with pain that changes with breathing, a negative troponin and this tracing is one thing; the same tracing in someone with typical pain, or after exertional syncope, or with a family history of sudden death, forces you to think again. And a measurement trap this very record reveals: the database labels it with left ventricular hypertrophy, and the Sokolow index reads 36.5 mm, above the threshold. But he is a 24-year-old man, lean and athletic: a thin chest wall brings the electrodes closer to the heart and raises the voltage without a gram of extra muscle. Voltage criteria have poor specificity precisely in this population. What stands out in this case — J wave, high voltage, bradycardia at 48 — is the combination seen in the athlete\u2019s heart, and the three findings appear together for the same reason.',
      pt: 'A armadilha que mais custa é a da palavra «precoce». Soa a achado de gente jovem e saudável, e durante décadas ensinou-se como uma variante normal e nada mais. Desde 2008 sabe-se que não é assim tão simples: a onda J nas derivações INFERIORES, quando é grande e quando o ST que a segue é horizontal ou descendente em vez de ascendente, associa-se a mais risco de fibrilhação ventricular idiopática. A distinção prática não é «tem ou não tem onda J» mas ONDE está e O QUE faz o ST depois. Este doente tem o achado nas laterais com o ST a subir para uma T alta, que é a forma de baixo risco e de longe a mais frequente. A segunda armadilha é a do contexto, e vale para qualquer eletro: isto não se diagnostica a partir do papel. Um doente de 24 anos com dor que muda com a respiração, troponina negativa e este traçado é uma coisa; o mesmo traçado em alguém com dor típica, ou depois de um síncope de esforço, ou com antecedente familiar de morte súbita, obriga a pensar de novo. E uma armadilha de medição que este mesmo registo deixa ver: a base etiqueta-o com hipertrofia ventricular esquerda, e o índice de Sokolow dá 36,5 mm, acima do limiar. Mas é um homem de 24 anos, magro e desportista: a parede torácica fina aproxima os elétrodos do coração e sobe a voltagem sem um grama de músculo a mais. Os critérios de voltagem têm má especificidade precisamente nesta população. O que salta à vista neste caso — onda J, voltagem alta, bradicardia a 48 — é o conjunto que se vê no coração de atleta, e os três achados aparecem juntos pela mesma razão.',
    },
    action: {
      es: 'No se activa el laboratorio de hemodinamia. Lo que sí hay que hacer, y en este orden: repetir el electro y la troponina a las tres horas, porque lo que descarta un infarto no es un trazado sino la ausencia de CAMBIO —la repolarización precoz es estable de un electro a otro, y un infarto evoluciona—, y si existe un electro previo del paciente, compararlo, que vale más que cualquier criterio de forma. Si las dos troponinas son negativas y el electro es igual al de tres horas antes, con este cuadro clínico el paciente se va de alta. Después, dos cosas que se olvidan. Primera: dejarlo escrito y darle una copia del electro. Este hallazgo lo va a acompañar toda la vida y va a aparecer en cada guardia a la que entre; un trazado previo en el bolsillo evita la próxima activación innecesaria. Segunda: preguntar por lo único que cambiaría la conducta, que es la historia de síncope —sobre todo durante el esfuerzo o inmediatamente después— y la de muerte súbita en familiares de primer grado antes de los 40 años. Si alguna de las dos está presente, deja de ser un hallazgo incidental y se deriva a cardiología para estudio. No se restringe el deporte por este electro: la repolarización precoz aparece en el 20 al 40 % de los atletas y en la mayoría es un marcador de entrenamiento, no de enfermedad.',
      en: 'The cath lab is not activated. What does need doing, in this order: repeat the ECG and the troponin at three hours, because what rules out an infarct is not a tracing but the absence of CHANGE — early repolarization is stable from one ECG to the next, and an infarct evolves — and if a previous ECG of this patient exists, compare it, which is worth more than any morphological criterion. If both troponins are negative and the ECG is the same as three hours earlier, with this clinical picture the patient goes home. Then, two things that get forgotten. First: write it down and give him a copy of the ECG. This finding will accompany him all his life and will show up in every emergency department he walks into; a previous tracing in his pocket prevents the next unnecessary activation. Second: ask about the only thing that would change management, which is a history of syncope — especially during or immediately after exertion — and of sudden death in first-degree relatives before the age of 40. If either is present, it stops being an incidental finding and goes to cardiology for study. Sport is not restricted on account of this ECG: early repolarization appears in 20 to 40 % of athletes and in most of them it is a marker of training, not of disease.',
      pt: 'Não se ativa o laboratório de hemodinâmica. O que é preciso fazer, e por esta ordem: repetir o eletro e a troponina às três horas, porque o que exclui um enfarte não é um traçado mas a ausência de MUDANÇA — a repolarização precoce é estável de um eletro para o outro, e um enfarte evolui — e, se existir um eletro prévio do doente, compará-lo, que vale mais do que qualquer critério de forma. Se as duas troponinas forem negativas e o eletro for igual ao de três horas antes, com este quadro clínico o doente tem alta. Depois, duas coisas que se esquecem. Primeira: deixá-lo escrito e dar-lhe uma cópia do eletro. Este achado vai acompanhá-lo toda a vida e vai aparecer em cada urgência a que entre; um traçado prévio no bolso evita a próxima ativação desnecessária. Segunda: perguntar pelo único que mudaria a conduta, que é a história de síncope — sobretudo durante o esforço ou imediatamente depois — e a de morte súbita em familiares de primeiro grau antes dos 40 anos. Se alguma das duas estiver presente, deixa de ser um achado incidental e vai para cardiologia para estudo. Não se restringe o desporto por causa deste eletro: a repolarização precoce aparece em 20 a 40 % dos atletas e na maioria é um marcador de treino, não de doença.',
    },
  },

  {
    ...
    record: 'JS22392',
    age: 45, sex: 'M',
    vitals: { bp: '138/84', spo2: 98, rr: 16 },
    highlight: ['V2', 'V3', 'V4'],
    answer: 'hipopotasemia',
    metrics: { kind: 'uwave', leads: ['V2', 'V3', 'V4'] },
    // Medido: después de cada T hay una segunda joroba separada por un valle.
    // En V2 mide 4,3 mm, en V3 2,3 y en V4 2,1. La razón U/T es 0,61 · 0,98 ·
    // 1,60: en V4 la T mide 1,3 mm y la U 2,1, o sea que la U es más alta que la T. En V5 y V6 la T y la U están
    // FUNDIDAS y la medición devuelve «no se separa», que es la respuesta
    // correcta y no un cero.
    //
    // La otra mitad del cuadro: el ST desciende 1,5 · 1,8 · 1,9 mm en V3, V4 y
    // V5. FC 58, cv 0,000, QRS 96 ms, PR 292 ms con una P de 0,5 mm en II,
    // eje +88°, ruido 24 µV.
    //
    // El QTc de Bazett da 511 ms y es un artefacto del hallazgo: el método de
    // la tangente no distingue el final de la T del comienzo de la U, así que
    // mide un QU y lo llama QT. El caso lo explica en vez de informarlo.
    findings: {
    ...
    stem: {
      es: 'Hombre de 45 años que consulta por debilidad en las piernas y calambres desde hace cuatro días. Cuenta que hace tres semanas empezó un tratamiento para la presión y que además viene con diarrea desde el fin de semana. No tiene dolor en el pecho ni palpitaciones. En el examen está lúcido, con la fuerza algo disminuida en ambos muslos y los reflejos apagados. Le piden un electro mientras llega el laboratorio.',
      en: '45-year-old man who comes with weakness in his legs and cramps for the past four days. He mentions that three weeks ago he started treatment for blood pressure and that he has also had diarrhoea since the weekend. No chest pain, no palpitations. On examination he is alert, with somewhat reduced strength in both thighs and diminished reflexes. An ECG is ordered while the laboratory results are awaited.',
      pt: 'Homem de 45 anos que consulta por fraqueza nas pernas e cãibras há quatro dias. Conta que há três semanas começou um tratamento para a tensão e que além disso tem diarreia desde o fim de semana. Não tem dor no peito nem palpitações. No exame está lúcido, com a força algo diminuída em ambas as coxas e os reflexos apagados. Pedem-lhe um eletro enquanto chega o laboratório.',
    },
    options: [
      { id: 'hipopotasemia', label: { es: 'Hipopotasemia (onda U prominente)', en: 'Hypokalaemia (prominent U wave)', pt: 'Hipocaliemia (onda U proeminente)' } },
      { id: 'qtlargo', label: { es: 'QT largo adquirido', en: 'Acquired long QT', pt: 'QT longo adquirido' } },
      { id: 'isquemia', label: { es: 'Isquemia subendocárdica', en: 'Subendocardial ischaemia', pt: 'Isquemia subendocárdica' } },
      { id: 'digital', label: { es: 'Efecto digitálico', en: 'Digitalis effect', pt: 'Efeito digitálico' } },
    ],
    explain: {
      es: 'Mirá lo que hay entre dos QRS en V3: no una joroba, DOS. Primero la onda T, después un valle, y después una segunda onda del mismo tamaño. Esa segunda onda es la U, y normalmente es tan chica que no se ve. Medidas sobre el latido promedio, la U mide 4,3 mm en V2, 2,3 en V3 y 2,1 en V4. Pero el número que hace el diagnóstico no es ése sino la comparación con la T: en V4 la razón U/T da 1,60, o sea que la U es MÁS ALTA que la T. Eso pasa porque el potasio bajo hace dos cosas a la vez, y por eso hay que mirar las dos: agranda la U y aplana la T. Una sola de las dos cifras se puede explicar de otra manera; juntas, no. La segunda mitad del cuadro está en el segmento ST, que desciende 1,5 mm en V3, 1,8 en V4 y 1,9 en V5. Eso es lo que separa esto de una onda U prominente por bradicardia, que existe y es normal: ahí la U crece pero el ST está en su lugar. Y explica por qué la isquemia subendocárdica es la opción que más tienta — también da descenso del ST en las precordiales— pero no da esta segunda onda después de la T, y el paciente no tiene dolor. Lo del QT largo es la trampa fina y conviene desarmarla: el QTc de este trazado mide 511 ms y parece prolongado. No lo está. El método que lo mide busca dónde termina la onda T, y acá la T se continúa con la U sin volver del todo a la línea de base: lo que quedó medido es un QU, no un QT. Es el error clásico de este electro y lo comete cualquier equipo automático. No es efecto digitálico: ahí el ST no baja derecho sino que se hunde en cubeta por debajo del punto J y vuelve a subir, y acá el descenso es plano. Y el relato encaja: diurético empezado hace tres semanas más diarrea, que son las dos pérdidas de potasio más frecuentes juntas, con debilidad muscular y reflejos apagados.',
      en: 'Look at what lies between two QRS complexes in V3: not one hump, TWO. First the T wave, then a trough, then a second wave of the same size. That second wave is the U, and normally it is so small it goes unseen. Measured on the averaged beat, the U reads 4.3 mm in V2, 2.3 in V3 and 2.1 in V4. But the number that makes the diagnosis is not that one, it is the comparison with the T: in V4 the U/T ratio reads 1.60, meaning the U is TALLER than the T. That happens because low potassium does two things at once, and that is why both have to be looked at: it enlarges the U and flattens the T. Either figure alone can be explained another way; together, they cannot. The second half of the picture is in the ST segment, which is depressed 1.5 mm in V3, 1.8 in V4 and 1.9 in V5. That is what separates this from a prominent U wave due to bradycardia, which exists and is normal: there the U grows but the ST stays where it belongs. It also explains why subendocardial ischaemia is the most tempting option — it too depresses the ST in the chest leads — but it does not produce this second wave after the T, and the patient has no pain. The long QT is the fine trap and is worth dismantling: the QTc of this tracing measures 511 ms and looks prolonged. It is not. The method that measures it looks for where the T wave ends, and here the T runs into the U without fully returning to baseline: what got measured is a QU, not a QT. It is the classic error of this ECG and any automatic machine makes it. It is not digitalis effect: there the ST does not descend straight but sags below the J point in a scooped shape and rises again, and here the depression is flat. And the history fits: a diuretic started three weeks ago plus diarrhoea, the two commonest potassium losses together, with muscle weakness and diminished reflexes.',
      pt: 'Olhe o que há entre dois QRS em V3: não uma corcova, DUAS. Primeiro a onda T, depois um vale, e depois uma segunda onda do mesmo tamanho. Essa segunda onda é a U, e normalmente é tão pequena que não se vê. Medidas sobre o batimento médio, a U mede 4,3 mm em V2, 2,3 em V3 e 2,1 em V4. Mas o número que faz o diagnóstico não é esse mas a comparação com a T: em V4 a razão U/T dá 1,60, ou seja, a U é MAIS ALTA do que a T. Isso acontece porque o potássio baixo faz duas coisas ao mesmo tempo, e por isso há que olhar as duas: aumenta a U e aplana a T. Uma só das duas cifras pode explicar-se de outra maneira; juntas, não. A segunda metade do quadro está no segmento ST, que desce 1,5 mm em V3, 1,8 em V4 e 1,9 em V5. É isso que separa isto de uma onda U proeminente por bradicardia, que existe e é normal: aí a U cresce mas o ST está no seu lugar. E explica porque a isquemia subendocárdica é a opção que mais tenta — também dá descida do ST nas precordiais — mas não dá esta segunda onda depois da T, e o doente não tem dor. O QT longo é a armadilha fina e convém desmontá-la: o QTc deste traçado mede 511 ms e parece prolongado. Não está. O método que o mede procura onde termina a onda T, e aqui a T continua-se com a U sem voltar de todo à linha de base: o que ficou medido é um QU, não um QT. É o erro clássico deste eletro e comete-o qualquer equipamento automático. Não é efeito digitálico: aí o ST não desce direito mas afunda em concha abaixo do ponto J e volta a subir, e aqui a descida é plana. E a história encaixa: diurético começado há três semanas mais diarreia, que são as duas perdas de potássio mais frequentes juntas, com fraqueza muscular e reflexos apagados.',
    },
    pitfall: {
      es: 'La trampa que más daño hace es la del QT. El equipo automático informó 511 ms y eso dispara una cadena entera de decisiones equivocadas: suspender fármacos que no hay que suspender, pedir estudios que no hacen falta, pensar en un QT largo congénito. Pero el QT no está prolongado: lo que pasa es que la onda U es tan grande que el trazado no vuelve a la línea de base entre la T y la U, y el método de la tangente —que busca el final de la T— termina midiendo hasta el final de la U. Es un QU disfrazado de QT. La maniobra para no caer es mirar el trazado y preguntarse si lo que se está midiendo es UNA onda o DOS: si hay un valle en el medio, hay dos, y el QT termina en el valle. Y acá lo importante es que la confusión no es inocente en la otra dirección tampoco — la hipopotasemia SÍ produce arritmias ventriculares, así que el riesgo existe; lo que no existe es el QT largo que justificaría echarle la culpa a un fármaco. La segunda trampa es de método clínico: la onda U prominente no es sinónimo de hipopotasemia. Aparece también en la bradicardia marcada, en la hipertrofia ventricular izquierda, con algunos antiarrítmicos y en la hipomagnesemia, y aparece —chiquita— en gente sana con el corazón lento. Lo que hace fuerte a este caso no es la onda U sola sino el conjunto: U más alta que la T, ST descendido, PR largo, y una historia de diurético con diarrea. Y la tercera, que es la única que puede matar: el electro no mide el potasio. La correlación entre el número del laboratorio y lo que se ve en el trazado es mala, hay hipopotasemias graves con electros casi normales y al revés. Este electro obliga a pedir el potasio y a empezar a actuar; no lo reemplaza.',
      en: 'The trap that does most harm is the QT. The automatic machine reported 511 ms and that sets off a whole chain of wrong decisions: stopping drugs that need not be stopped, ordering tests that are not needed, thinking of a congenital long QT. But the QT is not prolonged: what happens is that the U wave is so large that the tracing does not return to baseline between the T and the U, and the tangent method — which looks for the end of the T — ends up measuring to the end of the U. It is a QU disguised as a QT. The manoeuvre to avoid this is to look at the tracing and ask whether what is being measured is ONE wave or TWO: if there is a trough in the middle, there are two, and the QT ends at the trough. And here the confusion is not innocent in the other direction either — hypokalaemia DOES produce ventricular arrhythmias, so the risk is real; what does not exist is the long QT that would justify blaming a drug. The second trap is one of clinical method: a prominent U wave is not synonymous with hypokalaemia. It also appears in marked bradycardia, in left ventricular hypertrophy, with some antiarrhythmics and in hypomagnesaemia, and it appears — small — in healthy people with a slow heart. What makes this case strong is not the U wave alone but the combination: U taller than the T, depressed ST, long PR, and a history of a diuretic with diarrhoea. And the third, the only one that can kill: the ECG does not measure potassium. The correlation between the laboratory number and what is seen on the tracing is poor; there are severe hypokalaemias with almost normal ECGs and the reverse. This ECG compels you to request the potassium and to start acting; it does not replace it.',
      pt: 'A armadilha que mais dano faz é a do QT. O equipamento automático informou 511 ms e isso dispara uma cadeia inteira de decisões erradas: suspender fármacos que não há que suspender, pedir exames que não fazem falta, pensar num QT longo congénito. Mas o QT não está prolongado: o que acontece é que a onda U é tão grande que o traçado não volta à linha de base entre a T e a U, e o método da tangente — que procura o fim da T — acaba a medir até ao fim da U. É um QU disfarçado de QT. A manobra para não cair é olhar o traçado e perguntar se o que se está a medir é UMA onda ou DUAS: se houver um vale no meio, são duas, e o QT termina no vale. E aqui o importante é que a confusão não é inocente na outra direção também — a hipocaliemia PRODUZ arritmias ventriculares, portanto o risco existe; o que não existe é o QT longo que justificaria culpar um fármaco. A segunda armadilha é de método clínico: a onda U proeminente não é sinónimo de hipocaliemia. Aparece também na bradicardia marcada, na hipertrofia ventricular esquerda, com alguns antiarrítmicos e na hipomagnesemia, e aparece — pequenina — em gente saudável com o coração lento. O que torna forte este caso não é a onda U sozinha mas o conjunto: U mais alta do que a T, ST descido, PR longo, e uma história de diurético com diarreia. E a terceira, a única que pode matar: o eletro não mede o potássio. A correlação entre o número do laboratório e o que se vê no traçado é má, há hipocaliemias graves com eletros quase normais e ao contrário. Este eletro obriga a pedir o potássio e a começar a atuar; não o substitui.',
    },
    action: {
      es: 'Se pide el potasio ya, y con él el magnesio y la función renal, pero no se espera el resultado cruzado de brazos: con este electro y esta historia se empieza a reponer. La vía depende de la gravedad y de los síntomas. Este paciente tiene debilidad muscular, o sea que ya hay repercusión: se repone por vía intravenosa, diluido y por bomba, con el electro monitorizado, y NO en bolo — el potasio rápido por vena periférica es una de las formas clásicas de matar a un paciente con la intención de curarlo. Si estuviera asintomático y el potasio fuera apenas bajo, la vía oral alcanza y es más segura. Lo que se olvida más seguido es el magnesio: con el magnesio bajo el riñón sigue perdiendo potasio y la reposición no levanta, así que hay que medirlo y corregirlo en paralelo o el potasio no sube por más que se insista. Después, la causa, que acá está a la vista y tiene dos partes: el diurético que empezó hace tres semanas —hay que revisar cuál es y ajustarlo o cambiarlo por uno ahorrador de potasio— y la diarrea, que se trata y se hidrata. Y mientras tanto, monitoreo del ritmo: la hipopotasemia con onda U es un terreno arritmogénico, y la arritmia que hay que temer es la torsión de puntas. Lo que NO hay que hacer es tratar el QT de 511 ms como si fuera un QT largo: no se suspende nada por esa cifra ni se piden estudios por ella, porque no es un QT.',
      en: 'Request the potassium now, and with it the magnesium and renal function, but do not wait for the result doing nothing: with this ECG and this history, replacement starts. The route depends on severity and symptoms. This patient has muscle weakness, so there is already repercussion: replace intravenously, diluted and by pump, with ECG monitoring, and NOT as a bolus — rapid potassium through a peripheral vein is one of the classic ways of killing a patient while meaning to cure him. If he were asymptomatic and the potassium only mildly low, the oral route suffices and is safer. What gets forgotten most often is the magnesium: with low magnesium the kidney goes on losing potassium and replacement does not take, so it has to be measured and corrected in parallel or the potassium will not rise however hard you push. Then the cause, which here is in plain sight and has two parts: the diuretic started three weeks ago — find out which one and adjust it or switch to a potassium-sparing agent — and the diarrhoea, which is treated and hydrated. And meanwhile, rhythm monitoring: hypokalaemia with a U wave is arrhythmogenic ground, and the arrhythmia to fear is torsades de pointes. What must NOT be done is to treat the 511 ms QT as a long QT: nothing is stopped and no tests are ordered on account of that figure, because it is not a QT.',
      pt: 'Pede-se o potássio já, e com ele o magnésio e a função renal, mas não se espera o resultado de braços cruzados: com este eletro e esta história começa-se a repor. A via depende da gravidade e dos sintomas. Este doente tem fraqueza muscular, ou seja, já há repercussão: repõe-se por via intravenosa, diluído e por bomba, com o eletro monitorizado, e NÃO em bólus — o potássio rápido por veia periférica é uma das formas clássicas de matar um doente com a intenção de o curar. Se estivesse assintomático e o potássio apenas baixo, a via oral chega e é mais segura. O que mais se esquece é o magnésio: com o magnésio baixo o rim continua a perder potássio e a reposição não pega, portanto há que medi-lo e corrigi-lo em paralelo ou o potássio não sobe por mais que se insista. Depois, a causa, que aqui está à vista e tem duas partes: o diurético que começou há três semanas — há que ver qual é e ajustá-lo ou trocá-lo por um poupador de potássio — e a diarreia, que se trata e se hidrata. E entretanto, monitorização do ritmo: a hipocaliemia com onda U é terreno arritmogénico, e a arritmia a temer é a torsade de pointes. O que NÃO se deve fazer é tratar o QT de 511 ms como se fosse um QT longo: não se suspende nada por esse valor nem se pedem exames por ele, porque não é um QT.',
    },
  },

  {
    ...
    record: 'JS12522',
    age: 42, sex: 'M',
    vitals: { bp: '104/62', spo2: 97, rr: 18 },
    highlight: ['II', 'V1'],
    answer: 'completo',
    metrics: { kind: 'dissociation' },
    // Medido: dos frecuencias en el mismo trazado. El ventrículo va a 52, con
    // los RR en 1152 · 1164 · 1188 · 1164 · 1172 · 1232 · 1128 ms (cv 0,010), y
    // la aurícula a 118 — razón 2,29. La onda P mide 107 µV y 68 ms y la ven
    // cinco derivaciones en la misma fase.
    //
    // La segunda prueba no comparte nada con la primera: el PR latido a latido
    // da 144 · 304 · 364 · 80 · 276 · 188 ms. Sin orden y sin repetirse.
    //
    // QRS 128 ms, eje −80°, QTc 397, ruido 33 µV.
    //
    // El PR del latido promedio informa 292 ms y ese número no existe: promediar
    // latidos alineados por la R destruye la P cuando la aurícula va por su
    // cuenta, así que lo que quedó promediado no es una onda P. El caso lo dice.
    findings: {
    ...
    stem: {
      es: 'Hombre de 42 años traído por su mujer después de desmayarse en la cocina. Cuenta que se fue de golpe, sin aviso, y que volvió en sí en menos de un minuto sin confusión. Es el segundo episodio en tres semanas; el primero lo atribuyó al calor. Hace un mes tuvo un cuadro de fiebre y dolores articulares que se le pasó solo. No toma medicación. Está lúcido, la presión es 104/62, y lo que llama la atención en el examen es que el pulso va a 52 y, cada tanto, se escucha un primer ruido mucho más fuerte que los demás.',
      en: '42-year-old man brought in by his wife after fainting in the kitchen. He says he went down suddenly, without warning, and came round in under a minute with no confusion. It is the second episode in three weeks; the first he put down to the heat. A month ago he had a bout of fever and joint pains that settled on its own. He takes no medication. He is alert, blood pressure 104/62, and what stands out on examination is that the pulse is 52 and that, every so often, a first heart sound is heard much louder than the rest.',
      pt: 'Homem de 42 anos trazido pela mulher depois de desmaiar na cozinha. Conta que caiu de repente, sem aviso, e que voltou a si em menos de um minuto sem confusão. É o segundo episódio em três semanas; o primeiro atribuiu-o ao calor. Há um mês teve um quadro de febre e dores articulares que passou sozinho. Não toma medicação. Está lúcido, a tensão é 104/62, e o que chama a atenção no exame é que o pulso vai a 52 e que, de vez em quando, se ouve um primeiro ruído muito mais forte do que os outros.',
    },
    options: [
      { id: 'completo', label: { es: 'Bloqueo AV completo con escape ventricular', en: 'Complete AV block with ventricular escape', pt: 'Bloqueio AV completo com escape ventricular' } },
      { id: 'union', label: { es: 'Ritmo de la unión', en: 'Junctional rhythm', pt: 'Ritmo juncional' } },
      { id: 'mobitz2', label: { es: 'Bloqueo AV de segundo grado, Mobitz II', en: 'Second degree AV block, Mobitz II', pt: 'Bloqueio AV de segundo grau, Mobitz II' } },
      { id: 'bradicardia', label: { es: 'Bradicardia sinusal', en: 'Sinus bradycardia', pt: 'Bradicardia sinusal' } },
    ],
    explain: {
      es: 'Hay DOS ritmos en este trazado y ninguno sabe del otro. El ventrículo va a 52, y va con una regularidad notable: los RR miden 1152, 1164, 1188, 1164, 1172 y 1232 ms, o sea que no varían casi nada. La aurícula va a 118. La razón entre las dos es 2,29. Esa sola frase —dos frecuencias distintas, cada una regular— es el bloqueo AV completo, porque quiere decir que NINGÚN impulso auricular está llegando abajo: si alguno pasara, el ventrículo se adelantaría y perdería su regularidad. La prueba que lo cierra está en el PR, y es independiente de todo lo anterior. Medido latido a latido, da 144, 304, 364, 80, 276 y 188 ms. No se alarga progresivamente —eso sería un Wenckebach— ni se mantiene fijo con alguna P que no conduce —eso sería un Mobitz II—: es azaroso. La onda P que precede a cada QRS simplemente no tiene nada que ver con ese QRS; cayó ahí por casualidad. Y hay un dato más que vale la pena leer al revés: la aurícula no va a 70, va a 118. Está taquicárdica. El paciente tiene el gasto cardíaco por el piso a 52 latidos por minuto y el nodo sinusal —que funciona perfecto— acelera todo lo que puede para compensar. Lo que no puede es hacer llegar ese mensaje abajo. Descartemos las otras tres. No es bradicardia sinusal: ahí habría una P antes de cada QRS a la misma distancia siempre, y una sola frecuencia. No es Mobitz II: ahí algunas P conducen con un PR CONSTANTE y otras no, y el PR de acá no es constante ni de lejos. Y lo del ritmo de la unión es la respuesta que da quien no vio las P: el QRS mide 128 ms, o sea que es ANCHO, y un escape de la unión baja por el His y las dos ramas y sale angosto. Un escape ancho a 52 nace por debajo del His, en el músculo ventricular.',
      en: 'There are TWO rhythms in this tracing and neither knows about the other. The ventricle runs at 52, and it does so with remarkable regularity: the RR intervals measure 1152, 1164, 1188, 1164, 1172 and 1232 ms, hardly varying at all. The atrium runs at 118. The ratio between them is 2.29. That sentence alone — two different rates, each of them regular — is complete AV block, because it means NO atrial impulse is getting through: if any did, the ventricle would come early and lose its regularity. The proof that closes it is in the PR, and it is independent of everything above. Measured beat by beat, it reads 144, 304, 364, 80, 276 and 188 ms. It does not lengthen progressively — that would be a Wenckebach — nor stay fixed with an occasional P that fails to conduct — that would be Mobitz II: it is random. The P wave preceding each QRS simply has nothing to do with that QRS; it landed there by chance. And there is one more figure worth reading backwards: the atrium is not at 70, it is at 118. It is tachycardic. The patient has his cardiac output on the floor at 52 beats a minute and the sinus node — which works perfectly — is speeding up as much as it can to compensate. What it cannot do is get that message downstairs. Let us rule out the other three. It is not sinus bradycardia: there would be a P before every QRS, always at the same distance, and a single rate. It is not Mobitz II: there some P waves conduct with a CONSTANT PR and others do not, and the PR here is nowhere near constant. And junctional rhythm is the answer given by whoever did not see the P waves: the QRS measures 128 ms, that is, it is WIDE, and a junctional escape travels down the His and both bundles and comes out narrow. A wide escape at 52 is born below the His, in ventricular muscle.',
      pt: 'Há DOIS ritmos neste traçado e nenhum sabe do outro. O ventrículo vai a 52, e vai com uma regularidade notável: os RR medem 1152, 1164, 1188, 1164, 1172 e 1232 ms, ou seja, quase não variam. A aurícula vai a 118. A razão entre as duas é 2,29. Essa frase sozinha — duas frequências diferentes, cada uma regular — é o bloqueio AV completo, porque quer dizer que NENHUM impulso auricular está a chegar abaixo: se algum passasse, o ventrículo adiantar-se-ia e perderia a sua regularidade. A prova que o fecha está no PR, e é independente de tudo o anterior. Medido batimento a batimento, dá 144, 304, 364, 80, 276 e 188 ms. Não se alarga progressivamente — isso seria um Wenckebach — nem se mantém fixo com alguma P que não conduz — isso seria um Mobitz II: é aleatório. A onda P que precede cada QRS simplesmente não tem nada a ver com esse QRS; caiu ali por acaso. E há mais um dado que vale a pena ler ao contrário: a aurícula não vai a 70, vai a 118. Está taquicárdica. O doente tem o débito cardíaco pelo chão a 52 batimentos por minuto e o nó sinusal — que funciona perfeitamente — acelera tudo o que pode para compensar. O que não consegue é fazer chegar essa mensagem abaixo. Excluamos as outras três. Não é bradicardia sinusal: aí haveria uma P antes de cada QRS sempre à mesma distância, e uma só frequência. Não é Mobitz II: aí algumas P conduzem com um PR CONSTANTE e outras não, e o PR daqui não é constante nem de longe. E o ritmo juncional é a resposta de quem não viu as P: o QRS mede 128 ms, ou seja, é LARGO, e um escape juncional desce pelo His e pelos dois ramos e sai estreito. Um escape largo a 52 nasce abaixo do His, no músculo ventricular.',
    },
    pitfall: {
      es: 'La trampa que más veces se cae es contar la frecuencia y quedarse ahí. «Bradicardia a 52» es verdad y no es el diagnóstico: la pregunta no es a qué velocidad va el ventrículo sino si la aurícula tiene algo que ver con él. Y para contestarla hay que buscar las P donde molestan, no donde convienen: acá miden 1 mm, y varias caen encima de una onda T y la deforman. El truco de cabecera es medir la distancia entre dos P que se vean bien y llevar esa medida, con un papel o un compás, a lo largo de toda la tira: las P están donde el compás dice que tienen que estar, aunque estén escondidas. Segundo, una trampa de la propia medición, y este caso la exhibe: el PR calculado sobre el latido promedio informa 292 ms. Ese número no existe. Promediar latidos alineados por la R conserva la P cuando cada P va pegada a su QRS, y la DESTRUYE cuando la aurícula va por su cuenta — que es justo lo que pasa acá. Un PR «de 292 ms» sobre un trazado disociado es un promedio de cosas distintas, no un intervalo. Por eso el panel muestra dos frecuencias y no un PR. Tercero, el ancho del QRS no es un detalle de descripción, es el pronóstico: un escape angosto nace en la unión, va a 40-60 y es relativamente estable; uno ancho como éste nace en el ventrículo, va a 20-40 y se para sin avisar. El síncope de este paciente es eso. Y cuarto, para que no parezca que se esconde: hay supradesnivel del ST de 4 a 5 mm en V3 y V4. No es un infarto. En esas derivaciones el QRS es profundamente negativo —S de 12 mm—, y un complejo ancho arrastra la repolarización en dirección contraria a su propia deflexión. El ST elevado sobre un QRS negativo es lo esperable; lo que haría pensar en isquemia sería un ST desviado hacia el MISMO lado que el QRS.',
      en: 'The trap most often fallen into is counting the rate and stopping there. "Bradycardia at 52" is true and is not the diagnosis: the question is not how fast the ventricle goes but whether the atrium has anything to do with it. And to answer it you have to look for the P waves where they are awkward, not where they are convenient: here they measure 1 mm, and several fall on top of a T wave and deform it. The bedside trick is to measure the distance between two clearly visible P waves and carry that measurement, with a piece of paper or a pair of calipers, along the whole strip: the P waves are where the calipers say they must be, even when hidden. Second, a trap of the measurement itself, which this case exhibits: the PR computed on the averaged beat reports 292 ms. That number does not exist. Averaging beats aligned on the R preserves the P when each P is tied to its QRS, and DESTROYS it when the atrium goes its own way — which is exactly what happens here. A PR "of 292 ms" on a dissociated tracing is an average of different things, not an interval. That is why the panel shows two rates and not a PR. Third, the width of the QRS is not a descriptive detail, it is the prognosis: a narrow escape is born in the junction, runs at 40-60 and is relatively stable; a wide one like this is born in the ventricle, runs at 20-40 and stops without warning. This patient\u2019s syncope is precisely that. And fourth, so that it does not look hidden: there is 4 to 5 mm of ST elevation in V3 and V4. It is not an infarct. In those leads the QRS is deeply negative — an S of 12 mm — and a wide complex drags repolarization in the direction opposite to its own deflection. ST elevation over a negative QRS is what is expected; what would suggest ischaemia is an ST shifted to the SAME side as the QRS.',
      pt: 'A armadilha em que mais se cai é contar a frequência e ficar por aí. «Bradicardia a 52» é verdade e não é o diagnóstico: a pergunta não é a que velocidade vai o ventrículo mas se a aurícula tem alguma coisa a ver com ele. E para responder há que procurar as P onde incomodam, não onde convêm: aqui medem 1 mm, e várias caem em cima de uma onda T e deformam-na. O truque de cabeceira é medir a distância entre duas P bem visíveis e levar essa medida, com um papel ou um compasso, ao longo de toda a tira: as P estão onde o compasso diz que têm de estar, ainda que escondidas. Segundo, uma armadilha da própria medição, e este caso exibe-a: o PR calculado sobre o batimento médio informa 292 ms. Esse número não existe. Promediar batimentos alinhados pela R conserva a P quando cada P vai colada ao seu QRS, e DESTRÓI-A quando a aurícula vai por sua conta — que é justamente o que acontece aqui. Um PR «de 292 ms» sobre um traçado dissociado é uma média de coisas diferentes, não um intervalo. Por isso o painel mostra duas frequências e não um PR. Terceiro, a largura do QRS não é um detalhe de descrição, é o prognóstico: um escape estreito nasce na junção, vai a 40-60 e é relativamente estável; um largo como este nasce no ventrículo, vai a 20-40 e para sem avisar. O síncope deste doente é isso. E quarto, para que não pareça que se esconde: há supradesnivelamento do ST de 4 a 5 mm em V3 e V4. Não é um enfarte. Nessas derivações o QRS é profundamente negativo — S de 12 mm — e um complexo largo arrasta a repolarização em direção contrária à sua própria deflexão. O ST elevado sobre um QRS negativo é o esperável; o que faria pensar em isquemia seria um ST desviado para o MESMO lado que o QRS.',
    },
    action: {
      es: 'Esto se interna, monitorizado, hoy. Un bloqueo AV completo con escape ANCHO y síncope es indicación de marcapasos, y la discusión no es si se pone sino cuándo y de qué tipo. Mientras tanto: vía, monitor, palas o parches de marcapasos transcutáneo puestos encima del paciente —no en el carro—, y atropina a mano sabiendo que probablemente no haga nada, porque la atropina actúa sobre el nodo AV y acá el bloqueo está por debajo; en un bloqueo infrahisiano puede incluso empeorar las cosas al acelerar la aurícula sin mejorar la conducción. Si se descompensa antes de llegar al marcapasos definitivo, va transcutáneo de puente y después transvenoso. Antes de implantar nada hay que buscar lo reversible, porque cambia la conducta por completo: repasar la medicación —betabloqueantes, verapamilo, diltiazem, digital, amiodarona—, pedir potasio y función tiroidea, y descartar isquemia aguda. Y en este paciente, con 42 años y un cuadro febril con artralgias hace un mes, hay que preguntar explícitamente por picadura de garrapata y por exposición en zona endémica: la enfermedad de Lyme causa bloqueo AV completo, es reversible con antibiótico, y se resuelve en días o semanas sin necesidad de marcapasos definitivo. Un bloqueo completo en alguien joven y sin cardiopatía obliga a pensar también en miocarditis, sarcoidosis y en las formas familiares. Se marcapasea igual si hace falta hemodinámicamente —eso no se discute—, pero el implante definitivo se decide después de descartar la causa tratable, no antes.',
      en: 'This is admitted, monitored, today. Complete AV block with a WIDE escape and syncope is an indication for pacing, and the discussion is not whether but when and what kind. Meanwhile: intravenous access, monitor, transcutaneous pacing pads on the patient — not on the trolley — and atropine to hand, knowing it will probably do nothing, because atropine acts on the AV node and here the block is below it; in infra-Hisian block it can even make things worse by speeding the atrium without improving conduction. If he decompensates before the definitive device, transcutaneous pacing bridges to transvenous. Before implanting anything you must look for the reversible causes, because they change management completely: review the medication — beta blockers, verapamil, diltiazem, digoxin, amiodarone — check potassium and thyroid function, and rule out acute ischaemia. And in this patient, 42 years old with a febrile illness and joint pains a month ago, ask explicitly about a tick bite and exposure in an endemic area: Lyme disease causes complete AV block, is reversible with antibiotics, and resolves in days to weeks without a permanent pacemaker. Complete block in someone young with no heart disease also obliges you to think of myocarditis, sarcoidosis and the familial forms. He is paced anyway if haemodynamics demand it — that is not up for discussion — but the permanent implant is decided after the treatable cause has been excluded, not before.',
      pt: 'Isto interna-se, monitorizado, hoje. Um bloqueio AV completo com escape LARGO e síncope é indicação de pacemaker, e a discussão não é se se põe mas quando e de que tipo. Entretanto: acesso venoso, monitor, pás ou elétrodos de pacing transcutâneo colocados sobre o doente — não no carro — e atropina à mão, sabendo que provavelmente não fará nada, porque a atropina atua sobre o nó AV e aqui o bloqueio está abaixo; num bloqueio infra-hissiano pode até piorar as coisas ao acelerar a aurícula sem melhorar a condução. Se descompensar antes do pacemaker definitivo, vai transcutâneo como ponte e depois transvenoso. Antes de implantar seja o que for há que procurar o reversível, porque muda a conduta por completo: rever a medicação — betabloqueadores, verapamilo, diltiazem, digoxina, amiodarona —, pedir potássio e função tiroideia, e excluir isquemia aguda. E neste doente, com 42 anos e um quadro febril com artralgias há um mês, há que perguntar explicitamente por picada de carraça e por exposição em zona endémica: a doença de Lyme causa bloqueio AV completo, é reversível com antibiótico, e resolve-se em dias ou semanas sem necessidade de pacemaker definitivo. Um bloqueio completo em alguém jovem e sem cardiopatia obriga a pensar também em miocardite, sarcoidose e nas formas familiares. Coloca-se pacing na mesma se for preciso hemodinamicamente — isso não se discute — mas o implante definitivo decide-se depois de excluída a causa tratável, não antes.',
    },
  },
